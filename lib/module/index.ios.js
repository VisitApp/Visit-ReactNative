...
const VisitHealthView = _ref => {
  ...
  const callSyncApi = useCallback(data => axios.post(`${baseUrl}/users/data-sync`, data, {
    headers: {
  ...
  const callEmbellishApi = useCallback(data => axios.post(`${baseUrl}/users/embellish-sync`, data, {
    headers: {
      Authorization: authToken
    }
